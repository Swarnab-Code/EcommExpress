				...
				<table className='w-full'>
					<thead>
						<tr className='border-b border-slate-900/20 text-gray-30 regular-14 xs:regular-16 text-start py-12'>
							...
							<th className='p-1 text-left'>Orders</th>
						...
						{orders.map((order, i) => (
							<tr key={i} className='border-b border-slate-900/20 text-gray-50 p-6 medium-14 text-left'>
								...
								<td className='p-1'>
									...
									<hr className='w-1/2' />
									<div className=''>
										<h5 className='medium-16'>{order.address.firstName + " " + order.address.lastName}</h5>
										<div>
								...
								<td className='p-1'>
									<select onChange={(event) => statusHandler(event, order._id)} value={order.status} className='bg-primary ring-1 ring-secondary text-sm max-w-20 xl:max-w-24'>
